...
```bash
# 一键全流程（自动转 CFR → 检测台词区 → OCR → 翻译 → 渲染，输出在 <视频名>_output/）
...
| `ocr.use_gpu` | 用 onnxruntime-gpu(CUDA) 跑 OCR（需 NVIDIA 显卡；安装 `onnxruntime-gpu>=1.24,<1.26`，勿装 ≥1.26，见「GPU 加速」版本说明）。**⚠️ GPU OCR 会漏检部分台词（实测），识别完整性优先建议保持 `false`（CPU 后端）** |
...
- **实测：某句台词始终识别不到/只见乱码时，先排查 GPU OCR 漏检（见「OCR→GPU 加速」），不要盲目降低采样间隔**——实测 `sample_step` 32~48 对 GPU 漏检的句子全部无效，改用 CPU 后端后同一步长即可稳定读出。
...
> ⚠️ **GPU OCR 存在漏检缺陷（重要踩坑，实测）**：onnxruntime-gpu(CUDA) 跑 RapidOCR 时，**文本检测会对部分台词间歇性漏检**——同一帧 CPU 后端能稳定读出（0.97 分），GPU 后端只在少数帧读出。实测 5060 Ti 上会出现整句台词被漏掉，且**降低采样间隔（32~48）无法弥补**（漏检与采样密度无关）。后果：`segments.json` 缺整句，翻译/成品直接缺句。
> **解决方案：OCR 用 CPU 后端**（`ocr.use_gpu: false`），24 核机器 `--jobs 8` 并行约 3~7 分钟/视频，识别完整性显著高于 GPU（实测同视频段数 197→237，漏掉的句子全部恢复）。**渲染/CFR 仍可继续用 GPU**（x264 编码 + NVDEC 硬解不受影响）。没有 NVIDIA 显卡时保持默认 `use_gpu: false`、`encoder: x264` 即可。
...
- **渲染视频 A 的同时，尝试并行处理视频 B 的 CFR/OCR/翻译**：渲染主要占用 NVENC 编码 + CPU 画字幕；CFR 走 NVENC（独立编码会话，实测对渲染影响小）；OCR 走 CPU/GPU 推理（可错峰）；翻译走 API（不占实例算力）。实测这种调度把整批总耗时省了约 1/3。
...
- 上传速度受**实例出口带宽**限制，且不同目标差异极大（实测恒源云 OSS 实例→云端可达 **70MB/s+**，百度网盘约 4.5MB/s、阿里云盘仅约 0.5MB/s），会员权益只影响下载端。**成品交付首选 OSS 中转**；实际速度波动大，先测速选优（见「成品交付」）。
...
> **注意oss可能已经登录，应当先检测登录状态(如用oss ls查看输出)，或者手机号已经自动输入，直接回车然后输入密码即可**
...
- **某句台词整句缺失 / 只见乱码**：优先怀疑 **GPU OCR 漏检**（onnxruntime-gpu + RapidOCR 会对部分台词间歇性漏检，实测同帧 CPU 能读出、GPU 读不出；降低采样间隔 32~48 均无效）。处理：把 `ocr.use_gpu` 改为 `false`（CPU 后端）重跑 `ocr --force`，识别完整度显著提升（实测段数 197→237）；如个别句子仍缺，用密集抽帧 OCR（0.3~0.5s/帧）确认真实文本后手动补入 `segments.json`
